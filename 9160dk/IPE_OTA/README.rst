...
This program runs runs a oneM2M compliant IPE program, communication to the cloud via MQTT over cellular.

The program also runs Bluetooth® Low Energy central program to run device management on the Raspberry Pi and ESP32.
...
* Subscribe and Update the ACME CSE for device managmement
...
You must program the *board controller* with the :ref:`hci-lpuart` sample first, before programming the main controller with the LTE Sensor Gateway sample application.
...
#. Build the :ref:`hci-lpuart` program for the nrf9160dk_nrf52840 build target and program the board controller with it.
...
After programming the board controller, you must program the main controller with the this program.

Program the main controller as follows:

1. Set the **SW10** switch, marked as *debug/prog*, in the **NRF91** position.
#. Build the IPE program (this program) for the nrf9160dk_nrf9160_ns build target and program the main controller with it.
...
************************
src contains muliple header and source files. Although the names are self-explanatory, there is some overlap between the files. Here are the general guidelines for which files accomplish what:
...
* ble.c/ble.h: Manages the upper layers of the BLE stack (52840 manages the lower layers). All connections, transmissions and reception over BLE is completed here.
...
* main.c/main.h: Runs initilaization for all parts of code as well as LTE and modem.
